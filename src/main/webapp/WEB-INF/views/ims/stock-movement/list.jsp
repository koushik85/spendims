			...
			<div class="main-content">
				...
				<div class="page-header">
					<div>
						...
						<div class="page-subtitle">Track all inventory IN / OUT
							transactions</div>
					</div>
					<a href="${pageContext.request.contextPath}/stock-movement/new"
				...
				<%-- Filter bar --%>
				<form action="${pageContext.request.contextPath}/stock-movement"
					method="get">
					<div class="filter-bar">
						...
						<div class="search-wrap">
							...
							<input type="text" id="searchInput"
								...
								onkeyup="filterTable('movementTable')">
						</div>
						...
						<select name="productId" onchange="this.form.submit()">
							...
							<c:forEach var="p" items="${products}">
								<option value="${p.id}"
									${filterProductId == p.id ? 'selected' : ''}>${p.name}</option>
							</c:forEach>
						</select>
						...
						<c:if test="${not empty filterProductId or not empty filterType}">
							<a href="${pageContext.request.contextPath}/stock-movement"
								class="btn-filter"> <svg width="12" height="12" fill="none"
				...
				<div class="table-card">
						...
						<tbody>
							<c:forEach var="m" items="${movements}" varStatus="loop">
								<tr>
									...
									<td style="font-weight: 500;">${m.product.name}
										<div
											style="font-size: 0.75rem; color: var(--color-text-muted);">
											${m.product.category.name}</div>
									</td>
									...
									<td
										style="color: var(--color-text-muted); font-size: 0.8rem; white-space: nowrap;">
										${m.movedAtFormatted}</td>
									...
									<td><c:choose>
											...
											<c:otherwise>
												<span
													style="font-style: italic; opacity: 0.4; font-size: 0.8rem;">—</span>
											</c:otherwise>
										</c:choose></td>
									<td>
										<div style="display: flex; gap: 6px; align-items: center;">
											<a
												href="${pageContext.request.contextPath}/stock-movement/edit/${m.id}"
											...
											<form
												action="${pageContext.request.contextPath}/stock-movement/delete/${m.id}"
								...
								<tr>
									<td colspan="8">
										<div class="empty-state">
											...
											<p>
												No movements found. <a
													href="${pageContext.request.contextPath}/stock-movement/new"
													style="color: var(--color-primary);">Record one now.</a>
											</p>
										</div>
									</td>
								</tr>
					...
					<c:if test="${not empty movements}">
						<div class="table-footer">
							Showing <span id="visibleCount">${movements.size()}</span> of
