			...
			<div class="main-content">
				...
				<div class="breadcrumb-bar">
					...
					<span class="sep">›</span> <a
						...
						class="sep">›</span> <span class="current">${empty product.id ? 'New Product' : 'Edit Product'}</span>
				</div>
				...
				<div class="page-header">
					<h2>
						...
						</span> ${empty product.id ? 'Add New Product' : 'Edit Product'}
					</h2>
					<div class="page-subtitle">${empty product.id ? 'Fill in the details to add a new product to inventory.' : 'Update the product details below.'}
					</div>
				</div>
				...
				<c:if test="${not empty product.id}">
					<div class="edit-mode-banner">
						...
						Editing: <strong>${product.name}</strong> &nbsp;·&nbsp; SKU:
						${product.sku} &nbsp;·&nbsp; ID: ${product.id}
					</div>
					<div class="field-hint" style="margin-bottom: 14px;">
						Master-derived fields (Name, SKU, Category, HSN) are locked. You
						can still update pricing, supplier, and description.</div>
				</c:if>
				...
				<div class="form-card">
					...
					<div class="form-card-body">
						...
						<form
							action="${pageContext.request.contextPath}/product/${empty product.id ? 'new' : 'edit/'.concat(product.id)}"
							method="post">
							...
							<c:if test="${empty product.id}">
								...
								<div class="form-grid">
									<div class="form-group span-2">
										<label for="masterProductId">Select From Master
											Product <span class="required">*</span>
										</label>
										<div class="select-wrapper">
											<select id="masterProductId" name="masterProductId" required
												<c:if test="${empty masterProducts}">disabled</c:if>>
												<option value="" disabled selected>Select a master
													product…</option>
										...
										<div class="field-hint">Users can create products only
											from the master list. Select one to auto-fill details and set
											store pricing.</div>
										<div class="field-hint" style="margin-top: 8px;">
											Product not listed? <a
												href="${pageContext.request.contextPath}/product/request-master">Request
												Add to Master List</a> · <a
												href="${pageContext.request.contextPath}/product/my-requests">View
												My Requests</a>
										</div>
										<c:if test="${empty masterProducts}">
											<div class="alert alert-warning"
												style="margin-top: 10px; margin-bottom: 0;">No active
												master products are available right now. Submit a request to
												add one.</div>
										</c:if>
									</div>
								</div>
							...
							<div class="form-grid">
								...
								<div class="form-group">
									...
									<input type="text" id="name" name="name"
										value="${product.name}" placeholder="e.g. Wireless Mouse"
										required <c:if test="${not empty product.id}">readonly</c:if>
										autofocus>
								</div>
								...
								<div class="form-group">
									...
									<div class="select-wrapper">
										<select id="category" name="category.id"
											...
											<option value="" disabled ${emptyproduct.category ? 'selected' : ''}>Select
												a category…</option>
								...
								<div class="form-group">
									...
									<div class="sku-wrapper">
										<input type="text" id="sku" name="sku" value="${product.sku}"
											...
											<c:if test="${not empty product.id}">readonly</c:if> required>
										<c:if test="${empty product.id}">
											<button type="button" id="generateSkuBtn"
												class="sku-generate-btn">↻ Generate</button>
										</c:if>
									</div>
								...
								<div class="form-group" style="position: relative;">
									...
									<input type="text" id="hsnSearch"
										...
										value="${product.hsn.hsnCode}" autocomplete="off"
										<c:if test="${empty product.id}">required</c:if>
										<c:if test="${not empty product.id}">readonly</c:if>>

									<input type="hidden" id="hsnId" name="hsn.id"
								...
								<div class="form-group">
									...
									<div class="input-prefix-wrapper">
										<input type="number" id="costPrice" name="costPrice"
											value="${product.costPrice}" placeholder="0.00" step="0.01"
											min="0" required>
									</div>
								</div>

								<div class="form-group">
									<label for="sellingPrice">Selling Price <span
										class="required">*</span></label>
									<div class="input-prefix-wrapper">
										<input type="number" id="sellingPrice" name="sellingPrice"
											value="${product.sellingPrice}" placeholder="0.00"
											step="0.01" min="0" required>
									</div>
									<div class="field-hint">Used to auto-fill order and
										invoice unit price.</div>
								</div>
								...
								<div class="form-group">
									...
									<div class="input-prefix-wrapper">
										<input type="number" id="mrp" name="mrp"
											value="${product.mrp}" placeholder="0.00" step="0.01" min="0"
											required>
									</div>
								</div>
								...
								<div class="form-group">
									...
									<div class="select-wrapper">
										<select id="supplier" name="supplier.id" required>
											<option value="" disabled ${emptyproduct.supplier ? 'selected' : ''}>Select
												a supplier...</option>
									...
									<c:if test="${empty suppliers}">
										<div class="field-hint">
											No suppliers found. <a
												href="${pageContext.request.contextPath}/supplier/new">Add
												a supplier first</a>.
										</div>
									</c:if>
								</div>
