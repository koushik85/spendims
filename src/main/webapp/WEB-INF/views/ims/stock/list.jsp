...
<body>
		...
		<div class="row g-0">
			...
			<div class="main-content">
				...
				<div class="page-header">
					<div>
						<h2>Stock</h2>
						<div class="page-subtitle">Monitor and manage inventory
							levels</div>
					</div>
					<a href="${pageContext.request.contextPath}/stock/new"
				...
				<div class="search-bar">
					...
					<input type="text" id="searchInput"
						...
						onkeyup="filterTable('stockTable')">
				</div>
				...
				<div class="table-card">
						...
						<tbody>
							<c:forEach var="s" items="${stocks}" varStatus="loop">
								<tr>
									...
									<td style="font-weight: 500;">${s.product.name}
										<div
											style="font-size: 0.75rem; color: var(--color-text-muted);">
											${s.product.category.name}</div>
									</td>

									<td><span style="font-weight: 600; font-size: 0.95rem;">${s.quantity}</span>
										<span
										style="font-size: 0.75rem; color: var(--color-text-muted);">
											units</span></td>
									...
									<td>
										<%-- Compute fill % capped at 100, show badge if low --%> <c:set
											var="pct"
											value="${s.minThreshold > 0 ? (s.quantity * 100 / s.minThreshold) : 100}" />
										<c:set var="cappedPct" value="${pct > 100 ? 100 : pct}" /> <c:choose>
									...
									<td>
										<div
											...
											<%-- Quick adjust +1 --%>
											<form
												action="${pageContext.request.contextPath}/stock/adjust/${s.id}"
												method="post" style="margin: 0;">
												<input type="hidden" name="delta" value="1"> <input
													type="hidden" name="note" value="Quick +1 adjustment">
												<button type="submit" class="btn-action btn-adjust-plus"
													title="Add 1 unit">
											...
											<form
												...
												<input type="hidden" name="delta" value="-1"> <input
													type="hidden" name="note" value="Quick -1 adjustment">
												<button type="submit" class="btn-action btn-adjust-minus"
													title="Remove 1 unit">
													<svg width="11" height="11" fill="none" viewBox="0 0 24 24"
														stroke="currentColor" stroke-width="2.5">
                                                    <path
											...
											<button type="button" class="btn-action btn-adjust-plus"
												...
												title="Restock / custom adjust">Restock</button>

											<%-- Edit --%>
											<a
												href="${pageContext.request.contextPath}/stock/edit/${s.id}"
											...
											<%-- Deactivate --%>
											<form
												action="${pageContext.request.contextPath}/stock/delete/${s.id}"
												method="post" style="margin: 0;"
								...
								<tr>
									<td colspan="7">
										<div class="empty-state">
											...
											<p>
												No stock entries found. <a
													href="${pageContext.request.contextPath}/stock/new"
					...
					<c:if test="${not empty stocks}">
						<div class="table-footer">
							Showing <span id="visibleCount">${stocks.size()}</span> of
							${stocks.size()} entries
						</div>
					</c:if>
				</div>
	...
	<div id="restockModal"
		...
		<div
			style="background: #fff; border-radius: 12px; padding: 28px 32px; width: 360px; box-shadow: 0 8px 32px rgba(0, 0, 0, 0.18);">
			<h3 style="margin: 0 0 4px; font-size: 1rem; font-weight: 600;">Restock
				/ Adjust</h3>
			<p id="restockProductName"
				style="font-size: 0.85rem; color: #64748b; margin: 0 0 20px;"></p>
			<form id="restockForm" method="post">
				<div style="margin-bottom: 14px;">
					<label
						style="font-size: 0.8rem; font-weight: 600; color: #475569; display: block; margin-bottom: 6px;">
						Quantity Change <span style="color: #dc2626;">*</span>
					</label> <input type="number" id="restockDelta" name="delta"
						placeholder="e.g. 50 (add) or -10 (remove)"
						style="width: 100%; padding: 8px 12px; border: 1px solid #e2e8f0; border-radius: 6px; font-size: 0.875rem;"
						required>
					<div style="font-size: 0.75rem; color: #94a3b8; margin-top: 4px;">Positive
						= add stock. Negative = deduct.</div>
				</div>
				<div style="margin-bottom: 20px;">
					<label
						...
						placeholder="e.g. Supplier delivery, Damage writeoff…"
						style="width: 100%; padding: 8px 12px; border: 1px solid #e2e8f0; border-radius: 6px; font-size: 0.875rem;">
				</div>
