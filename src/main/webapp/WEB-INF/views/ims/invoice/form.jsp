			...
			<div class="main-content">
				...
				<div class="breadcrumb-bar">
					...
					<span class="sep">›</span> <a
						...
						class="sep">›</span> <span class="current">${empty invoice.id ? 'New Invoice' : 'Edit Invoice'}</span>
				</div>

				<div class="page-header with-icon">
					<h2>${empty invoice.id ? 'Create Invoice' : 'Edit Invoice'}</h2>
					<div class="page-subtitle">${empty invoice.id ? 'Fill in customer, dates, and line items.' : 'Update the draft invoice.'}
					</div>
				</div>
				...
				<form
					action="${pageContext.request.contextPath}/invoice/${empty invoice.id ? 'new' : 'edit/'.concat(invoice.id)}"
					...
					<div class="form-card">
						...
						<div class="form-card-body">
							...
							<div class="form-row">
								...
								<div class="form-group">
									<label for="salesOrderId">Linked Order <span
										...
										<select id="salesOrderId" name="salesOrder.id">
											...
											<c:forEach var="ord" items="${orders}">
												<option value="${ord.id}"
													...
													<c:out value="${ord.orderNumber}" /> —
													<c:out value="${ord.customer.displayName}" />
												</option>
											</c:forEach>
										</select>
									</div>
							...
							<div class="form-row">
								<div class="form-group">
									<label for="invoiceDate">Invoice Date <span
										class="required">*</span></label> <input type="date" id="invoiceDate"
										name="invoiceDate"
										value="${not empty invoice.invoiceDate ? invoice.invoiceDate : ''}"
										required>
								</div>
								<div class="form-group">
									...
									<div class="select-wrapper">
										<select id="paymentMode" name="paymentMode" required>
											<c:forEach var="mode" items="${paymentModes}">
												<option value="${mode}"
													${invoice.paymentMode == mode ? 'selected' : ''}>
													${mode.label}</option>
											</c:forEach>
										</select>
									</div>
							...
							<div class="form-row">
								<div class="form-group">
									<label for="customerGstin">Customer GSTIN <span
										class="optional">optional</span></label> <input type="text"
										id="customerGstin" name="customerGstin"
										value="${invoice.customerGstin}" maxlength="15"
							...
							<div class="form-row">
								...
								<div class="form-group">
									<label for="termsAndConditions">Terms &amp; Conditions
										<span class="optional">optional</span>
									</label>
									<textarea id="termsAndConditions" name="termsAndConditions"
										rows="2">${invoice.termsAndConditions}</textarea>
								</div>
							</div>
								...
								<tbody id="itemsContainer">
									<c:choose>
										<c:when test="${not empty invoice.items}">
											<c:forEach var="item" items="${invoice.items}"
												...
												<tr data-row="${loop.index}">
													...
													<td class="td-hsn"><input type="text"
														name="itemHsnCode"
														value="<c:out value="${item.hsnCode}"/>" placeholder="HSN">
													</td>
										...
										<c:when test="${not empty prefillItems}">
											<c:forEach var="item" items="${prefillItems}"
												varStatus="loop">
												<tr data-row="${loop.index}">
													<td style="color: var(--color-text-muted); width: 28px;">${loop.index+1}</td>
													<td class="td-product"><select name="itemProductId"
													...
													<td class="td-desc"><input type="text"
														...
														placeholder="Description"></td>
													<td class="td-hsn"><input type="text"
														name="itemHsnCode" placeholder="HSN"></td>
													<td class="td-qty td-num"><input type="number"
														name="itemQuantity" value="${item.quantity}" min="1"
														required onchange="recalcTotals()"></td>
													<td class="td-price td-num"><input type="number"
